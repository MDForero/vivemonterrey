...
export default async function page() {
    ...
    // Los usuarios con cualquier permiso de lectura pueden acceder al dashboard
    if (!role || (!hasPermission(role, PERMISSIONS.BUSINESSES_READ) && 
                  !hasPermission(role, PERMISSIONS.USERS_READ) && 
                  !hasPermission(role, PERMISSIONS.PRODUCTS_READ))) {
        redirect('/unauthorized')
    }

    return (
        <div className="container mx-auto p-4">
            {/* Aviso para usuarios sin perfil */}
            {!profile && (
                <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="text-lg font-medium text-yellow-800">Perfil Incompleto</h3>
                            <p className="text-yellow-700">
                                Para acceder a todas las funciones del dashboard, necesitas completar tu perfil.
                            </p>
                            <Link 
                                href="/dashboard/account" 
                                className="inline-flex items-center mt-2 px-4 py-2 bg-yellow-600 text-white font-medium rounded-md hover:bg-yellow-700 transition-colors"
                            >
                                Completar Perfil
                            </Link>
                        </div>
            ...
            <div className="flex gap-4 justify-center flex-wrap">
                {hasPermission(role, PERMISSIONS.BUSINESSES_READ) && (
                    <Link href='/dashboard/negocios' className="w-32 aspect-video bg-green-700 text-white font-bold content-center text-center rounded-md p-2 hover:bg-green-800 transition-colors">
                        ...
                        {role === ROLES.ADMIN ? 'Todos los Negocios' : 'Mis Negocios'}
                    </Link>
                )}
                
                {hasPermission(role, PERMISSIONS.PROPERTIES_READ) && (
                ...
                {hasPermission(role, PERMISSIONS.EVENTS_READ) && (
                ...
                {hasPermission(role, PERMISSIONS.USERS_READ) && (
                    <Link href='/dashboard/usuarios' className="w-32 aspect-video bg-indigo-600 text-white font-bold content-center text-center rounded-md p-2 hover:bg-indigo-700 transition-colors">
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 mx-auto mb-2">
                            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                            <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                            <g id="SVGRepo_iconCarrier">
                                <path d="M16 7C16 9.20914 14.2091 11 12 11C9.79086 11 8 9.20914 8 7C8 4.79086 9.79086 3 12 3C14.2091 3 16 4.79086 16 7Z" fill="#f8f8f8"/>
                                <path d="M12 14C8.13401 14 5 17.134 5 21H19C19 17.134 15.866 14 12 14Z" fill="#f8f8f8"/>
                            </g>
                        </svg>
                        Usuarios
                    </Link>
            ...
            {/* Resumen de negocios del usuario (para clientes) */}
            {(role === ROLES.CLIENT || role === ROLES.USER) && profile?.businesses && profile.businesses.length > 0 && (
                <div className="mt-8">
                    <h2 className="text-xl font-semibold mb-4">Mis Negocios</h2>
                        ...
                        {profile.businesses.map((business) => (
                            <div key={business.id} className="bg-white rounded-lg shadow-md p-4 border">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <Link 
                                        href={`/dashboard/negocios/${business.name}`}
                                    ...
                                    {hasPermission(role, PERMISSIONS.PRODUCTS_UPDATE) && (
                                        <Link 
                                            href={`/dashboard/negocios/${business.name}/menu`}
                                    ...
                                    {hasPermission(role, PERMISSIONS.ROOMS_READ) && (
                                        <Link 
                                            href={`/dashboard/negocios/${business.name}/habitaciones`}
                                            className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-amber-100 text-amber-800 hover:bg-amber-200 transition-colors"
                                        >
                                            Habitaciones
                                        </Link>
                                    )}
