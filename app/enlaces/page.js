...
export default async function page() {
    ...
    const { data: negocios, error } = await supabase.from('businesses').select('name , logo')
    ...
    return <div className="flex justify-center flex-wrap gap-8">
            {negocios?.map((negocio) => <div key={negocio.name} className="flex flex-col items-center border-2 w-fit">
                ...
                <QrCode value={`vivemonterrey.com.co/enlaces/${negocio.name.split(" ").join('-')}`} logo={negocio.logo} />
            </div>
            )}
    </div>
}