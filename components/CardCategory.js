...
const CardCategory = (data) => {
  ...
  return (
    <Link href={name.split(' ').join('-')} className=" group w-52 h-52 border-2 relative overflow-hidden rounded-xl ">
      ...
      {imageUrl ? <Image loading='lazy' src={imageUrl} alt={`imagen que representa la categoría ${name}`}  width={0} height={0} className=" group-hover:scale-125 w-80 h-80 object-cover duration-200" /> : null}
